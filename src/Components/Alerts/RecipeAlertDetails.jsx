...
const RecipeAlertDetails = async ({ row, getPatientNameById }) => {
    const { paciente, medicacion, firma_medica, fecha_ultimo_laboratorio, proxima_fecha_empadronamiento, observaciones } = row;
    const medicationDetails = medicacion.map(med => `
    ...
    if (result.isConfirmed) {
        ...
        for (let i = 0; i < medicacion.length; i += itemsPerPage) {
            if (i !== 0) {
                doc.addPage(); 
            }
            addPatientInfo(doc);
            const medicationSubset = medicacion.slice(i, i + itemsPerPage);
